...
public class Permissions {
	...
	//默认通过的方法
	private static final String[] passs = {"sysUser.login"};
	...
	public BackModel validation(JSONObject json ){
		...
		if(userTokenData == null){
			...
			//验证权限是否存在
			JSONArray ps = userData.getJSONArray(Constant.NAME_USER_FUNC );
			...
			for(int i = 0; i<ps.size(); i++){
				...
				if(method.equals(permis ) ){
					...
					//刷新token有效时间
					redisTemplate.expire(tokenKey, sysUserService.getTimeout(term), TimeUnit.SECONDS);
					return null;
				}
			}
		}
